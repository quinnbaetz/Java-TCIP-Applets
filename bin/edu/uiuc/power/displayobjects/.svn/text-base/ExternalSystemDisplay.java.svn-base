...
public class ExternalSystemDisplay implements Renderable, ImageObserver {

    BufferedImage transparent;
    Point2D _ULCorner;
    
    public ExternalSystemDisplay(Point2D ULCorner) throws IOException {
    	_ULCorner = ULCorner;
        String path = "/applet_images/transmission tower small.png";
    ...
    public boolean render(Graphics2D g2d) {
		...
		g2d.translate(50-(textwidth/2),transparent.getHeight()/2.0 - textbounds.getHeight()/2.0);
		...
		g2d.drawString(text, 0, 0);		
		/*
		if (substationNumber >= 0) {
			g2d.setTransform(beforeText);
			g2d.translate(0,12 + textbounds.getHeight());
			text = "" + substationNumber;
			textbounds = fontnow.getStringBounds(text, frc);
			textwidth = textbounds.getWidth();
